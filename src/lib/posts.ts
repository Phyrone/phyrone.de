		...
		const parse_promise = (async () => {
			...
			try {
				...
				if (parsed && parsed.groups?.slug) {
					const data = {
						year: parsed.groups?.year
							? parseInt(parsed.groups.year)
							: undefined,
						month: parsed.groups?.month
							? parseInt(parsed.groups.month)
							: undefined,
					...
					const metadata_result = await PostMetadata.safeParseAsync(
						metadata_unsafe,
					);
					...
					(((posts[data.year ?? 0] ??= {})[data.month ?? 0] ??= {})[
						data.day ?? 0
					] ??= {})[
						data.slug
					] = path;
...
export async function get_post_metadata(
	key: string,
